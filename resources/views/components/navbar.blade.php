...
@php
    // Construimos el menú desde config para usar slugs/id reales
    $servicesNav = collect(config('servicios.services'))
        ->map(fn($s) => [
            'label' => $s['title'] ?? 'Service',
            'slug'  => $s['slug']  ?? $s['id'], // slug confiable
        ])
        ->values();
@endphp
        ...
        <div class="flex items-center justify-between h-16 lg:h-20">
            ...
            <div class="hidden lg:flex items-center space-x-8">
                ...
                <div class="relative" x-data="{ open: false }" @keydown.escape.stop="open=false">
                    ...
                    >
                        <!-- All Services -->
                        <a href="{{ route('services.index') }}" role="menuitem"
                        ...
                        <!-- Servicios individuales (slugs reales desde config) -->
                        @foreach ($servicesNav as $item)
                            <a href="{{ route('services.show', ['slug' => $item['slug']]) }}" role="menuitem" class="dropdown-item">
                                {{ $item['label'] }}
                            </a>
                        @endforeach
                    </div>
                ...
                <div class="relative" x-data="{ open: false }" @keydown.escape.stop="open=false">
                    ...
                    >
                        <button
                            @click="currentLanguage = currentLanguage === 'EN' ? 'ES' : 'EN'; open = false"
            ...
            <div class="flex flex-col space-y-4">
                ...
                <div>
                    ...
                    <div class="ml-4 mt-2 space-y-2">
                        ...
                        <!-- Servicios individuales (slugs reales) -->
                        @foreach ($servicesNav as $item)
                            <a href="{{ route('services.show', ['slug' => $item['slug']]) }}" role="menuitem" class="dropdown-item">
                                {{ $item['label'] }}
                            </a>
                ...
                <button
                    @click="currentLanguage = currentLanguage === 'EN' ? 'ES' : 'EN'; isMobileMenuOpen = false"
